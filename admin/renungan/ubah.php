...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($isi_ayat) || empty($referensi_ayat) || empty($tanggal_publish) || empty($link_sumber)) {
        ...
        if (empty($error)) {
            $stmt = mysqli_prepare($conn, "UPDATE tb_renungan SET isi_ayat = ?, referensi_ayat = ?, gambar_renungan = ?, tanggal_publish = ?, link_sumber = ? WHERE id_renungan = ?");
            mysqli_stmt_bind_param($stmt, "sssssi", $isi_ayat, $referensi_ayat, $nama_file, $tanggal_publish, $link_sumber, $id_renungan);
    ...
    <div class="admin-layout">
        ...
        <div class="main-content">
            ...
            <form action="ubah.php?id=<?= $data['id_renungan']; ?>" method="POST" enctype="multipart/form-data" class="form-admin">
                ...
                <input type="url" name="link_sumber" id="link_sumber" value="<?= htmlspecialchars($data['link_sumber']); ?>" required>
